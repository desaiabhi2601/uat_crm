<!-- begin:: Footer -->
		...
		<script>
			$(document).ready(function(){
				...
				//new code
				$('.add_mtc_table_data').click(function(){
					var quotationMstId = $(this).attr('quotation-mst-id') ;
					var quoteNo = $(this).attr('quote-no') ;
					var clientId = $(this).attr('client-id') ;
					var clientName = $(this).attr('client-name') ;
					var mtcType = $('select#mtc_type_'+quotationMstId).val();
					var assignedTo = $('select#assigned_to_'+quotationMstId).val();
					if(mtcType == '' || assignedTo == ''){
						swal({
				    		title: "Please select MTC type and Assigned To",
				      		icon: "warning",
				    	});
						return false;
					}
					$.ajax({
						type: 'POST',
						data: {call_type: 'add_mtc_data', mtc_type: mtcType, mtc_for: quoteNo, mtc_for_id: quotationMstId, mtc_company: clientName, mtc_company_id: clientId, assigned_to: assignedTo},
						url: '<?php echo site_url('quality/ajax_function'); ?>',
						dataType: 'JSON',
				...
				$('.update_mtc_status').click(function(){
					var mst_id = $(this).attr('mtc-mst-id');
					$.ajax({
						type: 'POST',
						data: {call_type: 'get_mtc_status', mtc_mst_id: mst_id},
						url: '<?php echo site_url('quality/ajax_function'); ?>',
						dataType: 'JSON',
						success: function(res){
							if(res.status == 'successful') {
								$('div#mtc_status_body').html('').html(res.html_body);
								$('input#update_mst_id').val(mst_id);
							}
						}
					});
				});	

				$('li.delete_mtc_listing').click(function(){
					var mst_id = $(this).attr('mtc-mst-id');
					$.ajax({
						type: 'POST',
						data: {call_type: 'delete_mtc_details', mtc_mst_id: mst_id},
						url: '<?php echo site_url('quality/ajax_function'); ?>',
						dataType: 'JSON',
						success: function(res){
							if(res.status == 'successful') {
								swal({
						    		title: "Listing is Deleted successfully",
						      		icon: "success",
						    	});
								setTimeout(function() { 
									location.reload();
				...
				$('tbody#pending_issues_data').on('click', 'input#mst_search_filter', function(){

					var status_value_search = $('#status_search').val();
					var mtc_type_search = $('#mtc_type').val();
					var mtc_for_search = $('#mtc_for').val();
					var company_name_search = $('#mtc_company_name').val();
					var assigned_search = $('#mtc_assigned').val();
					var creater_search = $('#mtc_creater').val();
					ajax_call_function({call_type: 'search_status', search_value: status_value_search, mtc_type: mtc_type_search, mtc_for:mtc_for_search, company_name: company_name_search, assigned: assigned_search, creater: creater_search}, 'mtc_list_tbody');
					
				});

				$('tbody#pending_issues_data').on('click', 'input#mst_reset_filter', function(){
					ajax_call_function({call_type: 'search_status', search_value: '', mtc_type: '', mtc_for: '', company_name: '', assigned: '', creater: ''}, 'mtc_list_tbody');
				});

				$('li.delete_mtc_file').click(function(){
					swal({
					  title: "Are you sure?",
					...
					.then((willDelete) => {
					  	if (willDelete) {
							var mtc_file_id = $(this).attr('mtc-file-id');
							ajax_call_function({call_type: 'delete_mtc_file', mtc_file_id: mtc_file_id}, 'mtc_file_list_delete_option');
					    	swal({
				...
				$('button#mtc_status_update').click(function(){
					var mst_id = $('input#update_mst_id').val();
					var made_by = $("input[name='made_flag']:checked").val();
					var quality_admin = $("input[name='checked_by_quality_admin']:checked").val();
					var quality_admin_comment = $("input#qa_comment").val();
					var super_admin = $("input[name='checked_by_super_admin']:checked").val();
					var super_admin_comment = $("input#sa_comment").val();
					$.ajax({
						type: 'POST',
						data: {call_type: 'update_mtc_status', mtc_mst_id: mst_id,made_by: made_by, quality_admin: quality_admin, quality_admin_comment:quality_admin_comment, super_admin: super_admin, super_admin_comment: super_admin_comment},
						url: '<?php echo site_url('quality/ajax_function'); ?>',
						dataType: 'JSON',
						success: function(res){
							swal({
					    		title: "MTC status updated successfully",
					      		icon: "success",
					    	});
						}
					});
				...
				function ajax_call_function(data, callType, url = '<?php echo base_url('quality/ajax_function'); ?>') {
					...
					$.ajax({
						...
						success: function(res){
							if(res.status == 'successful') {
								if(callType == 'mtc_list_tbody') {
									$('tbody#pending_issues_data').html('').html(res.html_body);
									$('#status_search').val(data.search_value);
									$('#mtc_type').val(data.mtc_type);
									$('#mtc_for').val(data.mtc_for);
									$('#company_name').val(data.company_name);
									$('#mtc_assigned').val(data.assigned);
									$('#mtc_creater').val(data.creater);
								}
								if(callType == 'mtc_file_list_delete_option') {
									// alert('Mtc file is deleted');
									// location.reload();
								}
							}
						}
				...
				$("#add_company #addCompany").validationEngine('attach', {
					onValidationComplete: function(form, status){
						if(status == true){
							cname = $("#add_company #company_name").val();
							$.ajax({
								type: 'POST',
								data: {search: cname},
								url: '<?php echo site_url('client/searchClients'); ?>',
								success: function(res){
									if(res != '[]'){
										alert('Adding company failed. "Company already exists."');
										return false;
									}else{
										$.ajax({
				...
				$('#quotation_form').on('change', '.products', function(){
					...
					if(show_act == true){
						$("#additional_comment_tr").show();
						$("#additional_comment").val('Final invoicing based on actual length shipped.');
					}else{
						$("#additional_comment_tr").hide();
				...
				<?php if($this->router->fetch_class() == 'quotations' && $this->router->fetch_method() == 'list') { ?>
					...
					var KTDatatablesDataSourceAjaxServer = function() {
						...
						var sales_person = '<?php echo $user_str; ?>';
						var country = '<?php echo $country_str; ?>';
						var region = '<?php echo $region_str; ?>';
						...
						var initTable1 = function() {
							...
							var dt = table.DataTable({
								...
								columns: [
									...
									{data: null, responsivePriority: -1, render: function(rowData){
					                    var gt = rowData.grand_total;
					                    return $.fn.dataTable.render.number(',', '.', 0, rowData.currency_icon).display(gt);
					                }},
								...
								initComplete: function() {
									var thisTable = this;
									// alert('ini');
									var rowFilter = $('<tr class="filter"></tr>').appendTo($(dt.table().header()));
									this.api().columns().every(function() {
											...
											switch (column.title()) {
												case 'Quote #':
													input = $(`<input type="text" data-col-index="` + column.index() + `" class="form-control kt-input" placeholder="Quote #">`);
													break;
												...
												case 'Assigned To':
													input = $(`<select class="form-control form-control-sm form-filter kt-input" id="assigned_to" title="Assigned To" data-col-index="` + column.index() + `">
												     <option value="">Select</option>`+sales_person +`</select>`);
												 	break;

												case 'Date':
													input = $(`<input type="text" data-col-index="` + column.index() + `" class="form-control kt-input hasdatepicker" placeholder="Date">`);
													break;	

												case 'Month':
													input = $(`<input type="text" data-col-index="` + column.index() + `" class="form-control kt-input" placeholder="Month">`);
													break;	

												case 'Week':
													input = $(`<input type="text" data-col-index="` + column.index() + `" class="form-control kt-input" placeholder="Week">`);
													break;	
												
												case 'Client':
													input = $(`<input type="text" data-col-index="` + column.index() + `" class="form-control kt-input" placeholder="Client">`);
													break;

												case 'Value':
													input = $(`<input type="text" data-col-index="` + column.index() + `" class="form-control kt-input" placeholder="Value">`);
													break;

												case 'Country':
													input = $(`<select class="form-control form-control-sm form-filter kt-input" id="country" title="Country" data-col-index="` + column.index() + `">
												     <option value="">Select</option>`+country+`</select>`);
												 	break;	
													
												case 'Reg':
													input = $(`<select class="form-control form-control-sm form-filter kt-input" id="region" title="Region" data-col-index="` + column.index() + `">
												     <option value="">Select</option>`+region +`</select>`);
												 	break;

												case 'FUDate':
													input = $(`<input type="text" data-col-index="` + column.index() + `" class="form-control kt-input hasdatepicker" placeholder="FUDate">`);
													break;	
												...
												case 'Imp':
													input = $(`<select class="form-control form-control-sm form-filter kt-input" id="sent_by" title="Importance" data-col-index="` + column.index() + `">
														<option value="">Select</option><option value="V">V</option><option value="H">H</option><option value="M">M</option><option value="L">L</option></select>`);
													break;

												case 'Status':
													input = $(`<select class="form-control form-control-sm form-filter kt-input" id="country" title="Status" data-col-index="` + column.index() + `">
												...
												// case 'WApp':
												// 	input = $(`<input type="text" data-col-index="` + column.index() + `" class="form-control kt-input" placeholder="Reg">`);
												// 	break;	
								...
								columnDefs: [
									{
										...
										render: function(data, type, full, meta) {
											...
											if(full.has_query == true){
												has_query = `<button class="btn btn-sm btn-clean btn-icon btn-icon-md purchase_query" title="Purchase Query" quote_id="`+full.rfq_id+`" query_id="`+full.query_id+`" query_type="purchase">
				                        			<i class="fa fa-question-circle" style="color: green;"></i>
				                        		</button>`;
											}

											if((<?php echo $this->session->userdata('role'); ?> == 8 || <?php echo $this->session->userdata('role'); ?> == 6 || <?php echo $this->session->userdata('role'); ?> == 5 || <?php echo $this->session->userdata('role'); ?> == 1) && full.purchase_order != '' && full.purchase_order != null){
												po_link = `<a href="<?php echo site_url('assets/purchase_orders/'); ?>`+full.purchase_order+`" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="View Purchase Order" target="_blank" >
					                        ...
					                        <button class="btn btn-sm btn-clean btn-icon btn-icon-md query" title="Sales Query" quote_id="`+full.quotation_mst_id+`" query_type="sales">
				                        		<i class="fa fa-question-circle" style="color: red;"></i>
				                        	</button>
					                        <button class="btn btn-sm btn-clean btn-icon btn-icon-md mtcTask" title="MTC Task" quote_id="`+full.quotation_mst_id+`" quote_no="`+full.quote_no+`" company_id="`+full.client_id+`" company="`+full.client_name+`"><i class="la la-tasks"></i></button>`+has_query+`
									...
									/*{
										targets: <?php if($this->session->userdata('role') == 1) echo '11'; else echo '10';?>,
										title: 'WApp',
										orderable: false,
										render: function(data, type, full, meta) {
									...
									{
										targets: <?php if($this->session->userdata('role') == 1) echo '10'; else echo '9';?>,
										title: 'Status',
										orderable: false,
										render: function(data, type, full, meta) {
											if(full.status == 'Open'){
									...
									},
									{
										...
										orderable: false,
										render: function(data, type, full, meta) {
											return full.follow_up_text+`<br/><br/><span style="font-weight: lighter; font-size: 10px;" class="last_contacted">`+full.last_followed+`</span>`;
										},
									},
									{
										targets: 1,
									...
									{
										targets: <?php if($this->session->userdata('role') == 1) echo '9'; else echo '8';?>,
										render: function(data, type, full, meta) {
											if(full.importance == 'L') {
												return '<i class="la la-battery-quarter" style="font-size: 25px;"></i>';
											}if(full.importance == 'M') {
												return '<i class="la la-battery-half" style="font-size: 25px;"></i>';
											}if(full.importance == 'H') {
												return '<i class="la la-battery-three-quarters" style="font-size: 25px;"></i>';
											}if(full.importance == 'V') {
												return '<i class="la la-battery-full" style="font-size: 25px;"></i>';
											}else{return full.importance; }
					...
					$("#invoice_table").on('click', '.query', function(){
						...
						$.ajax({
							...
							success: function(res){
								...
								if(resp != '' && <?php echo $this->session->userdata('user_id');?> == resp[0].raised_by){
									$("#query-popup #close_query").show();
									$("#query-popup #query_status").val(resp[0].query_status);
								}
								if(resp != ''){
									$("#query-popup #query_id").val(resp[0].query_id);
								}
			        ...
			        $("#invoice_table").on('click', '.purchase_query', function(){
						var quote_id = $(this).attr('quote_id');
						var query_type = $(this).attr('query_type');
						// alert(quote_id);
						$.ajax({
							...
							success: function(res){
								...
								if(resp != '' && <?php echo $this->session->userdata('user_id');?> == resp[0].raised_by){
									$("#pquery-popup #close_query").show();
									$("#pquery-popup #query_status").val(resp[0].query_status);
								}
								if(resp != ''){
					...
					var KTDatatablesDataSourceAjaxServer = function() {
						...
						var initTable1 = function() {
							...
							var dt = table.DataTable({
								...
								serverSide: true,
								ajax: {
									url: '<?php echo site_url('quotations/followup_data'); ?>',
									type: 'post',
									data: function(data){} 
								},
								columns: [
								...
								columnDefs: [
									{
										...
										render: function(data, type, full, meta) {
					                        ...
					                        <!--<button class="btn btn-sm btn-clean btn-icon btn-icon-md query" title="Query" quote_id="`+full.quotation_mst_id+`" query_id="`+full.query_id+`" query_type="`+full.query_type+`">
						...
						$.ajax({
							...
							success: function(res){
								...
								if(res != '[]'){
									...
									var client_name = '';
									for(key in resp){
										table += '<tr><td>'+parseInt(key+1)+'</td><td>'+resp[key].query_text+'</td>';
										// quote_no = resp[key].quote_no;
										// client_name = resp[key].client_name;
									}
									table += '</table>';
								...
								columnDefs: [
									{
										...
										render: function(data, type, full, meta) {
											...
											if(full.lead_dtl_id > 0){
												var connect_btn = `<button type="button" class="btn btn-sm btn-clean btn-icon btn-icon-md lead_connect pull-right" title="Contact" member_id="`+full.lead_dtl_id+`" lead_id="`+full.client_id+`">
													<i class="la la-comment"></i>
												</button>`;
											}

											return `<a href="<?php echo site_url('leads/addLeadDetails/'); ?>`+full.client_id+`" class="btn btn-sm btn-clean btn-icon btn-icon-md" target="_blank"><i class="la la-info-circle"></i></a>`+connect_btn+`<button type="button" class="btn btn-sm btn-clean btn-icon btn-icon-md createTask" title="Create Task" lead_id="`+full.client_id+`">
									...
									},
									{
										...
										render: function(data, type, full, meta){
											...
											return `<p>`+img+` 
												<strong style="margin-left: 5px;" class="imported">`+full.client_name+`</strong>
											</p>
									...
									},
									{
										...
										render: function(data, type, full, meta){
											...
											var stage = '<select class="stage_dropdown" id="dropdown'+full.client_id+'-'+full.lead_dtl_id+'" lead_mst_id="'+full.client_id+'" '+disabled+'><option value=""></option>';
				...
				<?php } else if($this->router->fetch_class() == 'leads' && $this->router->fetch_method() == 'primary_leads_list'){ ?>
					...
					Highcharts.chart('decision-maker-container', {
					    /*chart: {
					        type: 'bar'
					    },
					    title: {
					        text: 'Purchase Decision Maker'
					    },
					    xAxis: {
							...
							var table = $('#primary_leads_table').DataTable({
								...
								columnDefs: [
									<?php if($this->session->userdata('role') == 13){ ?>
									{
										...
										orderable: false,
										render: function(data, type, full, meta) {
											return `<a href="<?php echo site_url('leads/addPrimaryLeadDetails/'); ?>`+full.imp_id+`/`+full.data_category+`" class="btn btn-sm btn-clean btn-icon btn-icon-md" target="_blank"><i class="la la-info-circle"></i></a>
					                        `;
										},
									},
									{
									...
									},
									{
										...
										render: function(data, type, full, meta){
											...
											var stage = '<select class="stage_dropdown" di id="dropdown'+full.imp_id+'-'+full.lead_dtl_id+'" lead_mst_id="'+full.lead_mst_id+'" '+disabled+'><option value=""></option>';
											if(full.lead_stage == 1){
												stage += '<option value="1" selected="selected" id="grey">Stage 1</option>';
											}else{
												stage += '<option value="1" id="grey">Stage 1</option>';
											...
											switch(full.no_of_employees){
												case '1-9':
												case '10-25':
													company_size = '<i class="fas fa-user"></i>';
													break;

												case '25-50':
												case '50-100':
													company_size = '<i class="fas fa-user-friends"></i>';
													break;
												...
												case '500-1000':
												case '1000+':
													company_size = '<i class="fas fa-users"></i>';
													break;
											}

											company_size += ' <span style="font-size: 12px;">'+full.no_of_employees+'</span>';

											return stage+` <br/><span style="font-weight: lighter;" class="last_contacted">`+full.last_contacted+`</span><br/>`+company_size;
										}
									},
									<?php } else { ?>
									{
					...
					$("#primary_leads_table").on('click', '.viewImportStats', function(){
						...
						var rank1 = ''; var rank2 = ''; var rank3 = '';
						if(tableData.factor_value1 != null && tableData.factor_value1 != ''){
							rank1 = tableData.factor_value1;
						}
						if(tableData.factor_value2 != null && tableData.factor_value2 != ''){
							rank2 = tableData.factor_value2;
						}
						if(tableData.factor_value3 != null && tableData.factor_value3 != ''){
							rank3 = tableData.factor_value3;
						}
						var tableInfo = `
							...
							<table class="table table-bordered">
								<tr>
									...
									<td><b>Rank 1</b> - `+rank1+`</td>
									<td><b>Rank 2</b> - `+rank2+`</td>
									<td><b>Rank 3</b> - `+rank3+`</td>
								</tr>
								<tr>
									<th>No. of employees</th>
									<td>`+tableData.no_of_employees+`</td>
									<th>Last Buy</th>
									<td>`+tableData.LAST_PURCHASED+`</td>
								</tr>
								<tr>
									<th>Specific Product to Pitch</th>
									<td>`+tableData.product_pitch+`</td>
									<th>Margins</th>
									<td>`+tableData.margins+`</td>
								</tr>
								<tr>
									<th>Sales Notes</th>
									<td colspan="3">`+tableData.sales_notes+`</td>
								</tr>
								<tr>
									<th>Purchase Comments</th>
									<td colspan="3">`+tableData.purchase_comments+`</td>
								</tr>
							</table>`;
						$("#leadtable").html(tableInfo);
						...
						if(tableData.dm_graph != ''){
							var graph_data = [];
							var chart4 = $('#decision-maker-container').highcharts();
							Object.keys(tableData.dm_graph).forEach(function(key) {
								graph_data.push(tableData.dm_graph[key]);
							});
							var chart4 = $('#decision-maker-container').highcharts();
							...
							chart4.title.update({
								text: 'Decision Making Percentage'
							}, false);
						...
						<?php } else if($this->session->userdata('role') == 5){ ?>
							$.ajax({
								...
								success: function(res){
									...
									chart3.setTitle({text: "Yearly Buying Trends"});
				...
				<?php } else if($this->router->fetch_class() == 'leads' && $this->router->fetch_method() == 'addLeadDetails'){ ?>
					...
					$("#lead_connect").validationEngine({
						promptPosition: "inline"
					});

					$("#addLead").validationEngine('attach', {
						onValidationComplete: function(form, status){
							if(status == true){
								cname = $("#lead_name").val();
								if($("#lead_id").val() == ''){
									$.ajax({
										...
										success: function(res){
											if(res != '[]'){
												/*if(confirm('Adding company failed. "Company already exists."')){
													return true;
												}else{
													return false;
												}*/
												...
												var frm_data = $("#addLead").serialize();
												$.ajax({
													type: 'POST',
													data: frm_data,
													url: '<?php echo site_url('leads/addLeadDetails/'); ?>'+$("#lead_id").val(),
													success: function(res){
														window.location = "leads/hetregenous_leads/"+$("#source").val();
													}
												});
												return false;
											}
				...
				<?php } else if($this->router->fetch_class() == 'leads' && $this->router->fetch_method() == 'addPrimaryLeadDetails'){ ?>
					...
					$("#lead_add_member").click(function(){
						...
						$("#lead_member_grid tbody").append('<tr><td><input class="form-control validate[required]" type="text" name="name[]"></td><td><input class="form-control" type="text" name="designation[]"></td><td><input class="form-control validate[required]" type="text" name="email[]"></td><td><input class="form-control" type="text" name="mobile[]"></td><td><select name="is_whatsapp[]" class="form-control"><option value="">Select</option><option value="Yes">Yes</option><option value="No">No</option></select></td><td><input class="form-control" type="text" name="skype[]"></td><td><input class="form-control" type="text" name="telephone[]"></td><td><select name="main_buyer[]" class="form-control validate[required]"><option value="">Select</option><option value="Yes">Yes</option><option value="No">No</option></select></td><td>-</td><td>-</td><td><input type="text" name="decision_maker[]" class="form-control validate[custom[onlyNumberSp]]"></td><td><input type="hidden" name="other_member[]" value="" class="member_id"><button type="button" class="btn btn-sm btn-clean btn-icon btn-icon-md lead_removeBtn" title="Delete"><i class="la la-trash"></i></button></td></tr>');
					});
				...
				<?php } else if($this->router->fetch_class() == 'client'){ ?>
					...
					$("#addClient").validationEngine('attach', {
						onValidationComplete: function(form, status){
							if(status == true){
								cname = $("#addClient #company_name").val();
								if($("#addClient #client_id").length == 0){
									$.ajax({
										type: 'POST',
										data: {search: cname},
										url: '<?php echo site_url('client/searchClients'); ?>',
										success: function(res){
											if(res != '[]'){
												...
												var frm_data = $("#addClient").serialize();
												id = '';
												...
												$.ajax({
													type: 'POST',
													data: frm_data,
													url: '<?php echo site_url('client/addClients/'); ?>'+id,
													success: function(res){
														window.location = "client/client_list";
													}
												});
												return true;
											}
				...
				<?php if($this->router->fetch_class() == 'home' && $this->router->fetch_method() == 'calendar') { ?>
					...
					var KTCalendarBackgroundEvents = function() {
					    ...
					    return {
					        ...
					        init: function() {
					            ...
					            var calendarEl = document.getElementById('daily_task_calendar');
					            var calendar = new FullCalendar.Calendar(calendarEl, {
					                ...
					                events: [
					                    <?php foreach ($daily_tasks as $key => $value) { ?>
					                    	<?php if($user_dtl[0]['role'] == 5){ ?>
					                    	{
					                    		title: 'Counts',
					                    		start: '<?php echo $value['date']; ?>',
					                    		className: "fc-event-danger fc-event-solid-warning",
					                    		description: `CA - <?php if($value['ca'] == null){echo 0; }else {echo $value['ca']; }?> / CC - <?php if($value['cc'] == null){echo 0; }else {echo $value['cc']; }?> / RC - <?php if($value['rc'] == null){echo 0; }else {echo $value['rc']; }?> / WA - <?php if($value['wa'] == null){echo 0; }else {echo $value['wa']; }?> / QS - <?php if($value['qs'] == null){echo 0; }else {echo $value['qs']; }?>`
					                    	},
					                    	<?php } else if($user_dtl[0]['role'] == 6 || $user_dtl[0]['role'] == 8){ ?>
					                    		{
						                    		title: 'Counts',
						                    		start: '<?php echo $value['date']; ?>',
						                    		className: "fc-event-danger fc-event-solid-warning",
						                    		description: `RD - <?php if($value['rd'] == null){echo 0; }else {echo $value['rd']; }?> / RP - <?php if($value['rp'] == null){echo 0; }else {echo $value['rp']; }?> / QP - <?php if($value['qp'] == null){echo 0; }else {echo $value['qp']; }?>`
						                    	},
					                    	<?php } ?>
					                    	{
						                        ...
						                        description: `<?php echo $value['task_accomplished']; ?>`
						                    },
						                    {
						                        title: 'Work in Progress',
						                        start: '<?php echo $value['date']; ?>',
						                        className: "fc-event-info",
						                        description: `<?php echo $value['work_in_progress']; ?>`
						                    },
						                    {
						                        title: 'Next Day Plan',
						                        start: '<?php echo $value['date']; ?>',
						                        className: "fc-event-info",
						                        description: `<?php echo $value['plan_for_tomorrow']; ?>`
						                    },
					                    <?php } foreach($desktopTime as $dkey => $dvalue){ ?>
					                    	{
						                    	title: 'DT - <?php echo $dvalue; ?>',
						                        start: '<?php echo $dkey ?>',
						                        className: "fc-event-info",
						                        description: '<?php echo $dvalue; ?>'	
						                    },
					                    <?php } ?>
					...
					$("#view_user").change(function(){
						window.location.href = '<?php echo site_url('home/calendar/'); ?>'+$("#view_user").val();
					});

					$("#daily_task_btn").click(function(){
						$.ajax({
							...
							var table = $('#pq_client_list').DataTable({
								...
								columnDefs: [
									...
									},
									{
										...
										title: 'Comments',
										render: function(data, type, full, meta){
											return full.comments+`<br/><br/><span style="font-weight: lighter; font-size: 10px;" class="last_contacted">`+full.last_contacted+`</span>`;
										}
									}
								]
							});
							...
							var table = $('#mtc_list').DataTable({
								...
								columnDefs: [
									...
									},
									{
										targets: 4,
										render: function(data, type, full, meta) {
											if(full.made_flag == '' || full.made_flag == null){
												return `N`;
											}else{
									...
									},
									{
										targets: 5,
										render: function(data, type, full, meta) {
											if(full.checked_by_quality_admin == '' || full.checked_by_quality_admin == null){
												return `N`;
											}else{
									...
									},
									{
										targets: 6,
										render: function(data, type, full, meta) {
											if(full.checked_by_super_admin == '' || full.checked_by_super_admin == ''){
												return `N`;
											}else{
				...
				<?php } if($this->router->fetch_class() == 'procurement') { ?>
					<?php if($this->router->fetch_method() == 'addRFQ') { ?>
									...
									success: function(res){
										...
										Object.keys(resp).forEach(function(key) {
											leads += `<div style="padding: 5px; width: 90%; cursor: pointer" onclick="filldata(`+resp[key].client_id+`, '`+resp[key].client_name+`', '`+resp[key].client_rank+`', '`+resp[key].last_purchased+`', '`+resp[key].client_source+`')">
												`+resp[key].client_name+` (`+resp[key].country+`)
											</div>`;
										});
						...
						$("#add_company #rfq_addCompany").validationEngine('attach', {
							onValidationComplete: function(form, status){
								if(status == true){
									cname = $("#rfq_addCompany #company_name").val();
									$.ajax({
										type: 'POST',
										data: {search: cname},
										url: '<?php echo site_url('client/searchClients'); ?>',
										success: function(res){
											if(res != '[]'){
												...
												$.ajax({
													url: "<?php echo site_url('client/addClientAjax'); ?>",
													data: $("#add_company #rfq_addCompany").serialize(),
													type: "POST",
													success: function(res){
														var resp = $.parseJSON(res);
														var client_id = resp.client_id;
					...
					<?php } if($this->router->fetch_method() == 'rfq_list') { ?>
									...
									initComplete: function() {
										...
										this.api().columns().every(function() {
											...
											switch (column.title()) {
												...
												case 'Quotation #':
													input = $(`<select class="form-control form-control-sm form-filter kt-input" id="country" title="Quote Status" data-col-index="` + column.index() + `">
															<option value="">Select</option>
									...
									columnDefs: [
										{
											...
											render: function(data, type, full, meta) {
												var has_query = '';
												if(full.has_query == true){
													var has_query = `<button class="btn btn-sm btn-clean btn-icon btn-icon-md query" title="Sales Query" query_id="`+full.query_id+`" quote_id="`+full.quotation_mst_id+`" query_type="`+full.query_type+`">
						                        		<i class="fa fa-question-circle" style="color: red;"></i>
						                        	</button>`;
												}
												var view_quote = '';
												...
												var raise_query = `<button class="btn btn-sm btn-clean btn-icon btn-icon-md purchase_query" title="Purchase Query" query_id="`+full.query_id+`" quote_id="`+full.rfq_mst_id+`" query_type="`+full.query_type+`">
						                        		<i class="fa fa-question-circle" style="color: green;"></i>
						                        	</button>`;
												...
												<?php } else if($this->session->userdata('role') == 6){?>
													return `<a href="<?php echo site_url('procurement/addRFQ/'); ?>`+full.rfq_mst_id+`" class="btn btn-sm btn-clean btn-icon btn-icon-md" target="_blank" title="View RFQ"><i class="la la-info-circle"></i></a>`+highlight+`
														...
														<button type="button" data-toggle="modal" data-target="#notes-modal" class="btn btn-sm btn-clean btn-icon btn-icon-md notes" title="View Notes" rfq_id="`+full.rfq_mst_id+`">
															<i class="la la-sticky-note"></i>
														</button>`+view_quote+has_query+raise_query+`
												...
												<?php } else { ?>
													return `<a href="<?php echo site_url('procurement/addRFQ/'); ?>`+full.rfq_mst_id+`" class="btn btn-sm btn-clean btn-icon btn-icon-md" target="_blank" title="View RFQ"><i class="la la-info-circle"></i></a>`+highlight+`
														...
														<a href="<?php echo site_url('quotations/add/'); ?>`+full.quote_id+`/`+full.rfq_mst_id+`" class="btn btn-sm btn-clean btn-icon btn-icon-md" target="_blank" title="Make Quote"><i class="fa fa-file-invoice"></i></a>`+view_quote+has_query+raise_query;
												<?php } ?>
											},
										},
										{
						...
						$("#rfq_table").on('click', '.query', function(){
							var quote_id = $(this).attr('quote_id');
						...
						$("#rfq_table").on('click', '.purchase_query', function(){
							var quote_id = $(this).attr('quote_id');
							var query_id = $(this).attr('query_id');
							var query_type = $(this).attr('query_type');
							// alert(quote_id);
				...
				<?php } if($this->router->fetch_class() == 'proforma' && $this->router->fetch_method() == 'list') { ?>
					...
					var KTDatatablesDataSourceAjaxServer = function() {
						...
						var sales_person = '<?php echo $user_str; ?>';
						var purchase_person = '<?php echo $purchase_str; ?>';
						var country = '<?php echo $country_str; ?>';
						var region = '<?php echo $region_str; ?>';
						...
						var initTable1 = function() {
							...
							var dt = table.DataTable({
								...
								serverSide: true,
								ajax: {
									url: '<?php echo site_url('proforma/list_data'); ?>',
									type: 'post',
									data: function(data){} 
								},
								columns: [
									...
									{data: 'username', responsivePriority: -1},
									<?php } ?>
									{data: 'purchased_by', responsivePriority: -1},
									{data: 'date', responsivePriority: -1},
									{data: 'client_name', responsivePriority: -1},
									{data: null, responsivePriority: -1, render: function(rowData){
					                    var gt = rowData.grand_total;
								...
								initComplete: function() {
									...
									this.api().columns().every(function() {
											...
											switch (column.title()) {
												case 'Proforma #':
													input = $(`<input type="text" data-col-index="` + column.index() + `" class="form-control kt-input" placeholder="Proforma #">`);
													break;
												...
												case 'Purchase Person':
													input = $(`<select class="form-control form-control-sm form-filter kt-input" id="assigned_to" title="Assigned To" data-col-index="` + column.index() + `">
												     <option value="">Select</option>`+purchase_person +`</select>`);
												 	break;
								...
								columnDefs: [
									{
										...
										render: function(data, type, full, meta) {
											...
											if((<?php echo $this->session->userdata('role'); ?> == 8 || <?php echo $this->session->userdata('role'); ?> == 7 || <?php echo $this->session->userdata('role'); ?> == 6 || <?php echo $this->session->userdata('role'); ?> == 5 || <?php echo $this->session->userdata('role'); ?> == 1) && full.purchase_order != '' && full.purchase_order != null){
												po_link = `<a href="<?php echo site_url('assets/purchase_orders/'); ?>`+full.purchase_order+`" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="View Purchase Order" target="_blank" >
						                            <i class="fa fa-file-pdf-o"></i>
						                        </a>`;
											}else{
												po_link = `<button class="btn btn-sm btn-clean btn-icon btn-icon-md uploadPO" title="Upload Purchase Order" quote_id="`+full.quotation_mst_id+`">
				                        	...
				                        	<button class="btn btn-sm btn-clean btn-icon btn-icon-md query" title="Sales Query" quote_id="`+full.quotation_mst_id+`" query_id="`+full.query_id+`" query_type="`+full.query_type+`">
				                        		<i class="fa fa-question-circle" style="color: red;"></i>
				                        	</button>`+po_link;
										},
									},
					...
					$("#proforma_table").on('click', '.query', function(){
						var quote_id = $(this).attr('quote_id');
						var query_type = $(this).attr('query_type');
						// alert(quote_id);
						$.ajax({
			        ...
			        $("#proforma_table").on('click', '.uploadPO', function(){
			        	var quote_id = $(this).attr('quote_id');
			        	$("#po_form #quote_id").val(quote_id);
			        	$("#po-popup").modal('show');
			        });


			        $('#po_form').validationEngine('attach', {
						onValidationComplete: function(form, status){
							if(status == true){
								return true;
							}
			    ...
			    <?php } if($this->router->fetch_class() == 'quotations' && $this->router->fetch_method() == 'query_list') { ?>
			    	"use strict";
					var KTDatatablesDataSourceAjaxServer = function() {
						...
						var initTable1 = function() {
							...
							var dt = table.DataTable({
								...
								serverSide: true,
								ajax: {
									url: '<?php echo site_url('quotations/query_list_data/'.$query_type.'/'.$query_status); ?>',
									type: 'post',
									data: function(data){} 
								},
								columns: [
									{data: 'record_id'},
									{data: 'query_for', responsivePriority: -1},
									/*<?php if($this->session->userdata('role') == 1) { ?>
									{data: 'username', responsivePriority: -1},
									<?php } ?>*/
									{data: 'sender', responsivePriority: -1},
									{data: 'recepient', responsivePriority: -1},
									{data: 'raised_on', responsivePriority: -1},
									{data: 'Actions', responsivePriority: -1},
								],
								initComplete: function() {
									var thisTable = this;
								...
								columnDefs: [
									{
										...
										orderable: false,
										render: function(data, type, full, meta) {
											if(full.query_type == 'purchase'){
												return `<button class="btn btn-sm btn-clean btn-icon btn-icon-md purchase_query" title="Sales Query" quote_id="`+full.query_for_id+`" query_id="`+full.query_id+`" query_type="`+full.query_type+`">
					                        		<i class="fa fa-question-circle" style="color: green;"></i>
					                        	</button>`;	
											}else{
												return `<button class="btn btn-sm btn-clean btn-icon btn-icon-md query" title="Sales Query" quote_id="`+full.query_for_id+`" query_id="`+full.query_id+`" query_type="`+full.query_type+`">
					                        		<i class="fa fa-question-circle" style="color: red;"></i>
					                        	</button>`;
											}
					...
					$("#query_table").on('click', '.query', function(){
						var quote_id = $(this).attr('quote_id');
						var query_type = $(this).attr('query_type');
						// alert(quote_id);
						$.ajax({
			        ...
			        $("#query_table").on('click', '.purchase_query', function(){
						var quote_id = $(this).attr('quote_id');
						var query_type = $(this).attr('query_type');
						// alert(quote_id);
						$.ajax({
				...
				$("#user_noti").click(function(){
					$.ajax({
						url: '<?php echo site_url('home/getNotifications'); ?>',
						success: function(res){
							var notify_str = '';
							if(res != ''){
								var resp = $.parseJSON(res);
								var i = 1;
								Object.keys(resp.list).forEach(function(key){
									notify_str += `
								...
								$("#notify_count").html(resp.count);
							}else{
								notify_str = '<p>No Notifications...</p>';
							}
							$("#notify_div").html(notify_str);
						}
					});
				});
					...
					if(search != ''){
						$.ajax({
							...
							success: function(res){
								...
								var leads = '';
								Object.keys(resp).forEach(function(key) {
									if(resp[key].origin == 'Hetro'){
										leads += '<div style="padding: 5px; width: 90%; cursor: pointer"><a href="<?php echo site_url('leads/addLeadDetails/'); ?>'+resp[key].client_id+'" target="_blank">'+resp[key].client_name+' ('+resp[key].country_name+')</a></div>';
									}else{
										leads += '<div style="padding: 5px; width: 90%; cursor: pointer"><a href="<?php echo site_url('client/addClients/'); ?>'+resp[key].client_id+'" target="_blank">'+resp[key].client_name+' ('+resp[key].country_name+')</div></a>';
									}
								});
								$("#company_result_client").html(leads).show();
							}
						});
					}
				});
				...
				$(".company_name").blur(function(){
					setTimeout(function(){
						$("#company_result_client").html('').hide();
					}, 500);
				});
			...
			function validateCompany(){
				var cname = $("#addClient #company_name").val();
				$.ajax({
					type: 'POST',
					data: {search: cname},
					url: '<?php echo site_url('client/searchClients'); ?>',
