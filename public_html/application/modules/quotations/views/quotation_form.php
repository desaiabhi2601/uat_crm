					...
					<?php echo form_open('', array("class" => "kt-form kt-form--label-right", "id" => "quotation_form", "enctype" => "multipart/form-data")); ?>
						<div class="kt-portlet__body">
							...
							<div class="form-group row">
								...
								<?php if($this->session->userdata('role') == 5 || $this->session->userdata('role') == 1){ $col='col-lg-3'; }else{ $col='col-lg-6'; } ?>
											...
											<?php 
												if(!empty($clients)){
													foreach ($clients as $key => $value) {
														$selected = '';
														if(isset($quote_details) && $quote_details[0]['client_id'] == $value['client_id']) {
															$selected='selected="selected"';
														}else if(!isset($quote_details) && isset($rfq_details) && $rfq_details[0]['rfq_company'] == $value['client_id'] && $rfq_details[0]['client_source'] == 'main'){
															$selected='selected="selected"';
														}
														echo '<option '.$selected.' value="'.$value['client_id'].'">'.ucwords(strtolower($value['client_name'])).' ('.$value['country'].')</option>';
													}
												}
											?>
										</select>
